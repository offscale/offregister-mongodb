...
def install0(**kwargs):
    installed = lambda: run("dpkg-query --showformat='${Version}' --show mongodb-org", quiet=True)
    ...
    def mongodb_apt_init():
        if kwargs['VERSION'] == '3.4':
            ...
            append('/etc/apt/sources.list.d/mongodb-org-3.4.list',
            ...
            sudo('rm /etc/apt/sources.list.d/mongodb-org*')
            append('/etc/apt/sources.list.d/mongodb-org-3.6.list',
                   'deb [ arch=amd64,arm64 ] https://repo.mongodb.org/apt/ubuntu xenial/mongodb-org/3.6 multiverse',
                   use_sudo=True)
        else:
            raise NotImplementedError("MongoDB version {}; update the package, or use '3.4' | '3.6'".format(kwargs['VERSION']))
